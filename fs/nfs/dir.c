...
void nfs_prime_dcache(struct dentry *parent, struct nfs_entry *entry)
{
	...
	if (dentry != NULL) {
		if (nfs_same_file(dentry, entry)) {
			nfs_refresh_inode(dentry->d_inode, entry->fattr, NULL);
			goto out;
		} else {
			d_drop(dentry);
			dput(dentry);
				...
				struct nfs4_label *label)
{
	...
	if (dentry->d_inode)
		goto out;
	if (fhandle->size == 0) {
		error = NFS_PROTO(dir)->lookup(NFS_SERVER(dir)->client, dir, &dentry->d_name, fhandle, fattr, NULL);
		if (error)
			goto out_error;
	}
	nfs_set_verifier(dentry, nfs_save_change_attribute(dir));
