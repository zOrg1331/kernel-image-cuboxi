...
struct rbd_device {
	...
	int			obj_len;
	char			obj_md_name[RBD_MAX_MD_NAME_SIZE]; /* hdr nm. */
			   ...
			   struct ceph_options *opt)
{
	...
	rbd_dev->client = ceph_create_client(opt, rbd_dev);
 ...
 */
static struct rbd_client_node *__get_client_node(struct ceph_options *opt)
{
	struct rbd_client_node *client_node;
			  ...
			  char *options)
{
	...
	client_node = __get_client_node(opt);
	if (client_node) {
	...
	ret = rbd_init_client(rbd_dev, opt);
	if (ret < 0)
		goto out_free;
...
static int rbd_init_disk(struct rbd_device *rbd_dev)
{
	...
	if (rbd_dev->client->options)
		snap = rbd_dev->client->options->snap;
			     ...
			     const char *buf, size_t count)
{
	...
	rc = ceph_pg_poolid_by_name(osdc->osdmap,
				     rbd_dev->pool_name);
	if (rc < 0)
		goto err_out_client;
	rbd_dev->poolid = rc;
...
MODULE_DESCRIPTION("rados blockd device");
