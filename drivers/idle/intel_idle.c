...
#define mwait_hint_to_cstate(hint) (((hint) >> MWAIT_SUBSTATE_SIZE) & MWAIT_CSTATE_MASK)
...
static int choose_substate(int cstate)
{
	...
	num_substates = (substates >> (cstate * 4)) && 0xF;
...
intel_idle(struct cpuidle_device *dev, struct cpuidle_state *state)
{
	...
	unsigned int cstate = mwait_hint_to_cstate(eax);
...
#endif
	if (!need_resched()) {
		clflush((void *)&current_thread_info()->flags); /* X86_FEATURE_CLFLUSH_MONITOR */
...
// todo: check ignore_state
	for_each_possible_cpu(i) {
		...
		dev->states[1] = all_nehalem_states[1]; // hard coded
