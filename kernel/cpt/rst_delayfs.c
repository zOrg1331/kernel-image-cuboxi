...
static int path_lookup_mnt(struct vfsmount *mnt, const char *name,
...
static void rebind_unix_socket(struct vfsmount *rmnt,
		struct unix_bind_info *bi)
{
	int err;
	struct nameidata nd;

	err = path_lookup_mnt(rmnt, ((char *)bi->path) + bi->path_off, 0, &nd);
	if (err < 0) {
		printk("%s: lookup [%s] err %d\n", __func__, bi->path, err);
		return;
	}
